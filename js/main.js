...
require(['jquery','openapp','scrollUp'],function($,openapp){
	...
	openapp.allOpenApp('https://itunes.apple.com/cn/app/er-ge-dian-dian-jing-xuan/id1006637877?mt=8','http://android.myapp.com/myapp/detail.htm?apkName=com.mampod.ergedd');
    });
    //header部分，下载APP按钮    
    ...
    $('.SongPlayCount').each(function(){
        ...
        if(SongPlayCount>10000){
                if(SongPlayCount>999999){
                    // $(this).text('99k+');
                    $(this).text(parseFloat(SongPlayCount/10000).toFixed(0)+'万次');
                    if($(window).width()<768){
                        $(this).siblings('.glyphicon-play-circle').css('float','left');
                    }

                }else{
                    $(this).text(parseFloat(SongPlayCount/10000).toFixed(1)+'万次');
                }
            }
...
//捕获异常
requirejs(onErrot=function(err){
